...
//获取权限数组
var permissionArray=[];
function myConnetions(type, id) {
	//alert("type:"+type+",id:"+id);
	var shoucangimg = $("#shoucangimg");
	var imgPath = shoucangimg.prop("src");
	var imgName = imgPath.substr(imgPath.lastIndexOf("/") + 1, imgPath.length);
	if (imgName == 'prevshoucang.png') {
		if (confirm("是否收藏？")) {
			shoucangimg.attr('src', '../../img/dianjishoucang.png');
		}
	}
}
...
function netAvailable() {
	var flag = summer.netAvailable();
	var js = document.scripts;
	var jsPath;
	for (var i = js.length; i > 0; i--) {
		if (js[i - 1].src.indexOf("summer.js") > -1) {
...
function noNet(path) {
	var nothing = '<div class="notFound tc active"><img src="' + path + 'noconnection.png" alt="网络不给力" width="60%" style="margin-top:30%" /><p class="f14 mt30 mb10">当前网络不给力！<p><p class="f12 um-gray">请刷新试试~</p><div class="btnCon mt40 mb20"><a class="btn" id="search_btn" onclick="refresh()">刷新</a></div></div>'
	$(".um-content").after(nothing);
}

function noAccess(path) {
	var nothing = '<div class="notFound tc active"><img src="' + path + 'noaccess.png" alt="无权限" width="60%" style="margin-top:30%"/><p class="f14 mt30 mb10">当前网络不给力！<p><p class="f12 um-gray">请刷新试试~</p><div class="btnCon mt40 mb0"><a class="btn" id="search_btn" onclick="refresh()">刷新</a></div></div>'
	$(".um-content").after(nothing);
	setTimeout(function() {
...
function openNewView(modelName, linkUrl,modelid) {
console.log(permissionArray)
if(permissionArray.indexOf(modelid)>-1){
//可以打开
 summer.openWin({
		id : modelName,
		name : modelName,
...
//test
var ipUrl = "10.6.231.12";
//服务器IP
//var ipUrl = "172.100.4.46";
var port = "5050";
