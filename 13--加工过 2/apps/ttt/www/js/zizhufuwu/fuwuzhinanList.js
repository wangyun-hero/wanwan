...
summerready = function() {
	...
	listview.on("itemClick", function(sender, args) {
		...
		summer.openWin({
			"id" : 'fuwuzhinanDetail',
			"url" : 'html/zizhufuwu/fuwuzhinanDetail.html',
			"pageParam" : {
				"id" : items.id,
				"title" : items.title,
				"content" : items.content,
				"releasetime" : items.releasetime
			}
		});
...
function serviceData() {
	...
	if (searchValue == "" || searchValue == null) {
		parms = "{'pt_tableCode':'" + tab + "','po_releasetime':'DESC'" + pageParam + "}";
	} else {
		parms = "{'pt_tableCode':'" + tab + "','" + column + "':'" + searchValue + "','po_releasetime':'DESC'" + pageParam + "}";
	}
	//alert(parms);
	$service.callAction({
