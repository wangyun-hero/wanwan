...
summerready = function() {
	...
	listview.on("itemClick", function(sender, args) {
		...
		summer.openWin({
			"id" : 'xywpcjwtDetail',
			"url" : 'html/zizhufuwu/xywpcjwtDetail.html',
			"pageParam" : {
				"id" : items.id,
...
function serviceData() {
	...
	if (searchValue == "" || searchValue == null) {
		parms = "{'pt_tableCode':'" + tab + "','pe_l1code':'xyjg','po_id':'DESC'" + pageParam + "}";
	} else {
		parms = "{'pt_tableCode':'" + tab + "','pe_l1code':'xyjg','" + column + "':'" + searchValue + "','po_id':'DESC'" + pageParam + "}";
	}
	//alert(parms);
	$service.callAction({
